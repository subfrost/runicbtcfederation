...
import { buildCoinbaseToAddress, buildDefaultBlock } from "./utils/block-helpers";
...
describe("metashrew-runes", () => {
  ...
  it("Runestone transfer invalid funds", async () => {
    ...
    expect(resultAddress1.balanceSheet.length).equals(0, "address 1 should not have received runes from invalid rune transfer");
    const resultAddress2 = await runesbyaddress(program, TEST_BTC_ADDRESS2);
    console.log(resultAddress2.balanceSheet)
    expect(resultAddress2.balanceSheet.length).equals(0, "address 2 should not have received runes from invalid rune transfer");
  });
  ...
  it("index Runestone integration test", async () => {
    ...
    const pointer1 = 1
    const block1 = initCompleteBlockWithRuneEtching(outputs, pointer1, undefined, premineAmount)
    ...
    const block2 = transferRune([input], runeId, amount, outputIndexToReceiveRunes, [output, refundOutput], outputRunePointer)
    ...
    const block3 = transferRune([input2], runeId, amount, outputIndexToReceiveRunes, [refundOutput, output], 2)
